...
- [ ] `C319` `[new]` `[E2E]` **Agent Markdown is enabled when the setting is absent:** Cold startup with no `renderAgentMarkdown` key renders deterministic headings, emphasis, inline/fenced code, lists, GFM table content, and Unicode through the real ACP stack. User prompts and fenced code contents remain literal. _(E2E: `Feature.AgentMarkdown`.)_
- [ ] `C320` `[new]` `[E2E]` **Agent Markdown can be disabled before helper startup:** Explicit `renderAgentMarkdown: false` reaches the helper startup command and displays original Markdown markers and Unicode without changing user-prompt or code content. _(E2E: `Feature.AgentMarkdown`.)_
- [ ] `C321` `[new]` `[E2E]` **Live Markdown toggles preserve the conversation and process identities:** Saving false-to-true-to-false changes redraws the prior reply in place while preserving pane, helper, master, agent process, and ACP session identities. An unrelated hot settings event leaves raw mode unchanged, and a subsequent prompt uses the same session. _(E2E: `Feature.AgentMarkdown`.)_
- [ ] `C322` `[new]` `[E2E]` **Expanded Markdown history keeps table geometry after narrow resize:** Completed replies collapse and expand through mouse and keyboard, retain table words and Unicode after narrowing the real pane, and copy the selected displayed table cell using post-resize coordinates without resubmitting the prompt. _(E2E: `Feature.AgentMarkdown`.)_
- [ ] `C323` `[new]` `[E2E]` **Streaming Markdown reclassifies partial syntax before ordered tool output:** A deterministic ACP fixture holds incomplete and completed emphasis prefixes until the real pane is observed. Closing syntax updates the prior text before turn completion; text/tool/text ordering survives streaming and finalization, while user prompts and tool markers remain literal. _(E2E: `Feature.AgentMarkdown`.)_
- [ ] `C324` `[new]` `[E2E]` **Markdown Settings toggle is accessible and updates the existing reply:** The default-on Settings control exposes its localized accessible name, saves explicit false through the real UI, and redraws the existing reply as raw Markdown without replacing the helper or ACP session. _(E2E: `Feature.AgentMarkdown`.)_
- [ ] `C325` `[new]` `[E2E]` **CRLF Markdown preserves literal blank lines across the ACP boundary:** CR and LF split across ACP chunks preserve exactly one original blank line inside code and raw HTML blocks; changing to raw mode retains the same content and session identity. _(E2E: `Feature.AgentMarkdown`.)_
- [ ] `C326` `[new]` `[E2E]` **Narrow Markdown tables keep bordered columns and wrap cell text:** Narrowing the real agent pane retains table columns and horizontal row separators, wraps long words and Unicode inside cells without losing text, and keeps selection coordinates correct. Widening restores the original grid geometry without a new prompt or ACP session. _(E2E: `Feature.AgentMarkdown`.)_
- [ ] `C327` `[new]` `[E2E]` **Task-list headings preserve the helper while Markdown streams:** A task item reclassified as an underlined heading during ACP streaming keeps its checkbox and text without exiting the helper when heading markers are hidden. Ordinary tasks and code remain literal where required; completion, raw/rendered toggles and a follow-up prompt preserve the helper and ACP session. _(E2E: `Feature.AgentMarkdown`.)_
