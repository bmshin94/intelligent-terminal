...
//
// Build-time codegen for the WTA ETW provider and embedded translation catalogue.
//
...
//
// Telemetry scanning uses only std; localization reuses rust-i18n's parser.
...
fn main() {
    // Preserve the existing Debug executable reserve. Translation initialization
    // must also work on independently sized test and Tokio thread stacks.
    ...
    generate_localization(&manifest_dir, &out_dir);
...
fn generate_localization(manifest_dir: &Path, out_dir: &Path) {
    let source = manifest_dir.join("locales");
    println!("cargo:rerun-if-changed={}", source.display());
    let catalogue = rust_i18n_support::load_locales(
        source.to_str().expect("locale path must be Unicode"),
        |_| false,
    );
    assert!(
        catalogue
        ...
        "the source translation catalogue must not be empty"
    );

    // The stock macro emits one temporary map per translation in one initializer.
    // Static sorted slices avoid that unbounded Debug stack frame and allocations.
    let mut generated = String::from("static EMBEDDED_LOCALES: &[(&str, &[(&str, &str)])] = &[\n");
    for (locale, entries) in catalogue {
        writeln!(generated, "({locale:?}, &[").unwrap();
        for (key, value) in entries {
            writeln!(generated, "({key:?}, {value:?}),").unwrap();
        }
        generated.push_str("]),\n");
    }
    generated.push_str("];\n");

    let empty = out_dir.join("empty-i18n-catalogue");
    fs::create_dir_all(&empty).expect("create empty macro catalogue");
    writeln!(
        generated,
        "i18n!({:?}, fallback = \"en-US\", backend = localization::Embedded);",
        empty
            .to_str()
            .expect("generated locale path must be Unicode"),
    )
    .unwrap();
    fs::write(out_dir.join("localization_codegen.rs"), generated)
        .expect("write embedded translation catalogue");
}
